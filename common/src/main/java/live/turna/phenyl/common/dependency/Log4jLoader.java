...
public class Log4jLoader {
    ...
    private final transient NativeLogger Logger;
    ...
    public void onLoad() throws IOException {
        try {
            ...
            if (!libs.exists()) {
                if (!libs.mkdir()) {
                    throw new IOException("Failed creating library directory");
            ...
            for (Dependency dependency : dependencies) {
                ...
                for (DependencyRepository repo : DependencyRepository.values()) {
                    ...
                    if (!repo.download(dependency.getMavenRepoPath(), jarFile))
                        Logger.warn("Failed downloading dependency: " + jarFile.getPath());
                    if (!repo.download(dependency.getMavenRepoPath() + ".md5", md5File))
                        Logger.warn("Failed downloading dependency: " + md5File.getPath());
